...
class TinyGsmBG96 : public TinyGsmModem<TinyGsmBG96>,
 ...
 public:
  ...
  uint8_t wait_RDY(void) {
    uint8_t result;
    result = waitResponse(20000L, GF(GSM_NL "RDY"));
    return result;
  }
  ...
  uint8_t getServiceProviderName() {
    uint8_t spn = 0;
    sendAT(GF("+COPS?"));
    if (waitResponse(GF(GSM_NL "+COPS: ")) != 1) { return spn; }
    String resp = stream.readString();
    resp.toLowerCase();
    // Serial.println(resp);
    int sp = find_text("mobifone", resp);
    if (sp != -1) {
      ServiceProvider = "Mobifone";
      return 1;
    }
    sp = find_text("viettel", resp);
    if (sp != -1) {
      ServiceProvider = "Viettel";
      return 2;
    }
    ServiceProvider = "Not detect";
    // if(token[1]==String("VN Mobifone")){
    //   spn= 1;
    // }else if(token[1]==String("Viettel")) {
    //   spn= 2;
    // }
    return 0;
  }
  uint8_t initGPRS() {
    ...
    char apn_u[20];
    char apn_p[20];
    memset(apn_n, 0, sizeof(apn_n));
    memset(apn_u, 0, sizeof(apn_u));
    memset(apn_p, 0, sizeof(apn_p));
    // delay(200);
    sendAT(GF("E0"));
    waitResponse();
    if (!waitForNetwork()) {
      log_e("Wait For Network: Error");
      return 0;
    }
    uint8_t spn = getServiceProviderName();
    switch (spn) {
      case 0: {
      } break;
      case 1: {
        memcpy(apn_n, "m-wap", sizeof("m-wap"));
        memcpy(apn_u, "mms", sizeof("mms"));
        memcpy(apn_p, "mms", sizeof("mms"));
      } break;
      case 2: {
        memcpy(apn_n, "v-internet", sizeof("v-internet"));
        memcpy(apn_u, "", sizeof(""));
        memcpy(apn_p, "", sizeof(""));
      } break;
      default: {
        memcpy(apn_n, "internet", sizeof("internet"));
        memcpy(apn_u, "", sizeof(""));
        memcpy(apn_p, "", sizeof(""));
      } break;
    }
    log_d("APN: %s, user: %s, pass: %s", apn_n, apn_u, apn_p);

    if (!gprsConnect(apn_n, apn_u, apn_p)) {
      log_e("GPRS connect error");
      return 0;
    }
    log_e("GPRS connect OK");
    return 1;
  }
 ...
 protected:
  ...
  String readTime() {
    ...
    datas = "+QLTS";
    sendAT(GF(datas));
    if (waitResponse(2000L, GF(GSM_NL "+QLTS: \"")) != 1) { return ""; }
    String res;
    res = stream.readStringUntil('"');
    res.trim();
    return res;
 ...
 public:
  tm readTimeTm() {
    struct tm t   = {0};
    String    c_t = readTime();
    // log_d("time->%s", c_t.c_str());

    t.tm_year = c_t.substring(0, 2).toInt() + 100;
    t.tm_mon  = c_t.substring(3, 5).toInt() - 1;
    t.tm_mday = c_t.substring(6, 8).toInt();
    t.tm_hour = c_t.substring(9, 11).toInt() + 7;
    t.tm_min  = c_t.substring(12, 14).toInt();
    t.tm_sec  = c_t.substring(15, 17).toInt();
    return t;
  }
 ...
 protected:
  ...
  int16_t modemSend(const void* buff, size_t len, uint8_t mux) {
    ...
    while (curr_len < len) {
      int t_len = 1460;
      if (curr_len + 1460 < len) {
        curr_len += 1460;
      } else {
        curr_len += len % 1460;
        t_len = len % 1460;
      }
      sendAT(GF("+QISEND="), mux, ',', (uint16_t)t_len);
      if (waitResponse(GF(">")) != 1) { return 0; }
      // log_d("ADDR: %d,LEN: %d,  T_LEN: %d", curr_addr,len, t_len);
      stream.write(reinterpret_cast<const uint8_t*>(buff + curr_addr), t_len);
      stream.flush();
      if (waitResponse(GF(GSM_NL "SEND OK")) != 1) { return 0; }
      curr_addr += 1460;
    }
